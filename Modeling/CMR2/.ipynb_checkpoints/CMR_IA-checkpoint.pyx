...
class CMR2(object):
    ...
    def __init__(self, params, pres_mat, sem_mat, cue_mat=None, source_mat=None, task='FR', mode='IFR'):
        """
        ...
        if cue_mat is None: # [bj] input cue mat
            self.cues_nos = 0
        else:
            self.cues_nos = np.array(cue_mat, dtype=np.uint16)
        ...
        if task not in ('FR', 'CR','Recog'): # [bj] raise error for task parameter
        ...
        self.task = task  # [bj] Task type -> FR or CR or Recog
        ...
        # [bj] Determine and unpair when presented word pairs
        if self.pres_nos.ndim == 3:  # [bj] whether presented word-pairs
            self.word_pair = True
        else:
            self.word_pair = False
        if self.word_pair: # [bj] unpair word-pairs
        ...
        self.pres_nos_nonzero = self.pres_nos[self.nonzero_mask]
        # self.pres_nos_sorted = np.sort(self.pres_nos[self.nonzero_mask]) # [bj] also reduce dimension to 1D
        # self.pres_nos_unique = np.unique(self.pres_nos_sorted) # [bj] 1D, order in feature vector
        self.all_nos_sorted = np.sort(np.concatenate((self.pres_nos_nonzero,self.cues_nos),axis=None)) # [bj] pres and cue, also reduce to 1D
        self.all_nos_unique = np.unique(self.all_nos_sorted) # [bj] 1D, order in feature vector
        ...
        self.cues_indexes = np.searchsorted(self.all_nos_unique, self.cues_nos) # [bj]
        ...
        if self.nsources == 0:
            self.L_FC.fill(self.params['gamma_fc']) # jbg: if no source, uniformly gamma_fc
        else:
        ...
        if self.nsources == 0:
            self.L_CF.fill(self.params['gamma_cf']) # jbg: if no source, uniformly gamma_cf
        else:
    ...
    def run_trial(self):
        ...
        for self.serial_position in range(self.pres_indexes.shape[1]):
            ...
            if not self.nonzero_mask[self.trial_idx, self.serial_position].all:
                continue
    ...
    def present_item(self, item_idx, source=None, update_context=True, update_weights=True):
        ...
        if update_weights:
            ...
            if self.task == 'CR': # jbg: pair association!
                self.pair_association = np.dot(self.f, self.f.T)
    ...
    def cued_recall(self, cue_idx, time_limit=5000):
        ...
        self.ret_thresh[self.cues_indexes[self.trial_idx,cue_idx]] = np.inf
    ...
    def recognition(self, cue_idx):
        ...
        c_similarity = np.dot(self.c_old[:self.ntemporal].T, self.c_in[:self.ntemporal])
        if c_similarity >= self.params['c_thresh']:
            print("recall", cue_idx, "recognition judgement:\n","YES")  #
            self.rec_items[-1].append([1])
        else:
            print("recall", cue_idx, "recognition judgement:\n","NO")  #
            self.rec_items[-1].append([0])
