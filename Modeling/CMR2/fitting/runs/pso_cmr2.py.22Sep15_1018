...
from optimization_utils import get_data, obj_func, make_boundary
...
def pso(func, lb, ub, df, w2v, sources, swarmsize=100,
            ...
            for i in range(S):
                ...
                try:
                    ...
                    if not oob:
                        ...
                        err, stats = func(x[i, :], df, w2v, sources) ###
...
def run_pso(df, w2v, sources, sim_name=''):
    ...
    hard_bounds = False
      ...
      #    [ b_e,  b_r, g_fc, g_cf, p_s, p_d,  k,  e, s_fc, s_cf, b_rp,  o,  a, c_t,   l]
#     lb = [  .1,    0,    0,  .15,   0,   0,  0,  0,    0,   .5,    0,  5, .5,   0,   0]
#     ub = [  .9,    1,    1,  .85,   8,   5, .5, .25,   1,    3,    1, 20,  1,   1, .25]
    
    lb, ub, _ = make_boundary()
    ...
    xopt, fopt = pso(obj_func, lb, ub, df, w2v, sources, swarmsize=swarmsize, maxiter=n_iter,
...
if __name__ == "__main__":
    ...
    SIM = 'David'
#     wordpool_file = '../wordpools/wasnorm_wordpool.txt'
    w2v_file = '/home1/beigejin/Recog_David/data_als/semantic_mat_fasttext.npy'
...
#         data_pres = df.itemno.to_numpy()
#         sessions = df.sessions.to_numpy()
#         targets = df.yes.to_numpy()
...
#                     targets[key][subkey] = np.array(targets[key][subkey], dtype=float)
    ...
    run_pso(df, w2v, sources, sim_name=SIM)
